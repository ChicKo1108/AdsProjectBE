...
const { checkFieldPermissions } = require('../utils/permissionUtils');
 ...
 * 验证用户是否有权限操作请求中的字段
 */
const requireFieldPermissions = (req, res, next) => {
  if (!req.user) {
    logger.warn('权限验证失败：用户信息不存在');
    return res.status(401).json({ 
      code: -1, 
  ...
  // 超级管理员拥有所有权限
  if (req.user.role === 'super-admin') {
    logger.info(`超级管理员字段权限验证通过：${req.user.username}`);
    return next();
  }
  ...
  if (requestedFields.length === 0) {
    return next(); // 没有字段需要验证
  }

  const permissionCheck = checkFieldPermissions(req.user, requestedFields);
  
  if (!permissionCheck.hasPermission) {
    logger.warn(`字段权限验证失败：用户 ${req.user.username} 无权操作字段: ${permissionCheck.restrictedFields.join(', ')}`);
    return res.status(403).json({ 
      code: 403, 
      success: false, 
      message: `权限不足，无法操作以下字段: ${permissionCheck.restrictedFields.join(', ')}`, 
      data: {} 
    });
  }

  logger.info(`字段权限验证通过：用户 ${req.user.username} 可以操作字段: ${requestedFields.join(', ')}`);
  next();
};
...
/**
 * 检查用户是否有权访问指定账户（任何级别的权限）
 */
const requireAccountAccess = (accountIdParam = 'accountId') => {
  return requireAccountPermission('ad_operator', accountIdParam);
};

/**
 * 检查用户是否为账户管理员
 */
const requireAccountAdmin = (accountIdParam = 'accountId') => {
  return requireAccountPermission('site_admin', accountIdParam);
};
 ...
 * @param {string} accountIdParam - 账户ID参数名
 */
const requireAccountPermissionWithFields = (requiredRole = 'ad_operator', accountIdParam = 'accountId') => {
  return (req, res, next) => {
    // 先验证账户权限
    requireAccountPermission(requiredRole, accountIdParam)(req, res, (err) => {
      if (err) return next(err);
      
      // 再验证字段权限
      requireFieldPermissions(req, res, next);
    });
  };
};
...
module.exports = {
  ...
  requireAccountPermissionWithFields
};